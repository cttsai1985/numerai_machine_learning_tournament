...
import ds_utils
from ds_utils import EnsembleSolutionConfigs, EnsembleSolution
...
def parse_commandline() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ensemble Solver", add_help=True, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def main(args: argparse.Namespace):
    ...
    ensemble_configs = EnsembleSolutionConfigs(
        root_resource_path=root_resource_path, configs_file_path=args.configs, eval_data_type=args.eval_data_type, )

    output_data_path: str = ensemble_configs.output_dir_
    Path(output_data_path).mkdir(parents=True, exist_ok=True)
    solver = EnsembleSolution.from_configs(args=args, configs=ensemble_configs, output_data_path=output_data_path)
