...
if "__main__" == __name__:
    ...
    ret = ret.groupby(col_metric).apply(lambda x: x.reset_index()).reindex(columns=["target", _column_score, "path"])
    ...
    for _metric, _cutoff in _metric_selections:
        ...
        allow_list.append(sub["path"].loc[mask].tolist())

    all_list = set(ret['path'].tolist())
    allow_list = list(map(lambda x: set(x), allow_list))
    for i in allow_list:
        all_list = all_list & i
        logging.info(f"count: {len(all_list)}")

    ret = ret.loc[ret['path'].isin(all_list)]
