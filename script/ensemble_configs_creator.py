...
def parse_commandline() -> argparse.Namespace:
    ...
    default_ensemble_script_filepath: str = "./ensemble.py"
    ...
    parser.add_argument(
        "--ensemble-script", type=str, default=default_ensemble_script_filepath, help="ensemble script")
    parser.add_argument(
    ...
    parser.add_argument("--max-model-weight", type=float, default=.51, help="upper bound of weights in model ensemble")
...
if "__main__" == __name__:
    ...
    existing_configs: List[str] = list(map(
        lambda x: SolutionConfigs(root_resource_path=root_resource_path, configs_file_path=x), existing_config_files))

    output_dirs: str = list(map(lambda x: str(Path(x.output_dir_).absolute()), existing_configs))

    active_output_dirs: List[str] = list(filter(lambda x: os.path.exists(x) and os.path.isdir(x), output_dirs))
    config_file_output_dict: Dict[str, str] = {
        k: v for k, v in zip(output_dirs, existing_config_files) if k in active_output_dirs}
    ...
    combinations_all_ranges = list(filter(
        lambda x: pd.Series(x).value_counts(normalize=True).max() < _args.max_model_weight, combinations_all_ranges))
    ...
    logging.info(f"load template run script: {run_template}")
    ...
    for config_filepath in configs_all_ranges:
        _run_compute_template = {
            ...
            "script_file": _args.ensemble_script,
            "script_type": "python"
        }
