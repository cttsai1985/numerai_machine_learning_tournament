...
def compute(root_prediction_path: str) -> pd.DataFrame:
    output_file_path: str = os.path.join(root_prediction_path, ft.prediction_diagnostics_filename_template)
    ...
    summary.to_csv(output_file_path.format(eval_type="tournament", filename_extension="csv"), )
    summary.to_parquet(output_file_path.format(eval_type="tournament", filename_extension="parquet"), )
    return summary
